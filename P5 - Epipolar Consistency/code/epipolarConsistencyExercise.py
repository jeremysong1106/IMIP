...
def load_projections(projection_filename, shape):
    ...
    projections.append(np.load(projection_filename + '1.npy'))  # TODO
    projections = np.reshape(projections, [2, shape[0], shape[1]])

    # make sure that the projections are returned with correct dimensions
    # assert shape[0] == projections.shape[0]# TODO
    # projections = np.reshape(shape,projections)
    # print(max(projections[0].shape))
    return projections
...
def load_geometry(matrices_filename):
    ...
    projection_matrices.append(np.load(matrices_filename + '0.npy'))  # TODO
    ...
    # print(projection_matrices.shape)
